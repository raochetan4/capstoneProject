...
bg <- readRDS("bigram.RData"); tg <- readRDS("trigram.RData"); qd <- readRDS("quadgram.RData"); pt <- readRDS("pentgram.RData")
...
names(pt)[names(pt) == 'word1'] <- 'w1'; names(pt)[names(pt) == 'word2'] <- 'w2'; names(pt)[names(pt) == 'word3'] <- 'w3'
names(pt)[names(pt) == 'word4'] <- 'w4';names(pt)[names(pt) == 'word5'] <- 'w5';
...
predictWord <- function(the_word) {
        ...
        ############### check pentgram
        else if (n >= 4) {the_word <- as.character(tail(the_word,4)); functionPentgram(the_word)}
}
########################################################################
functionBigram <- function(the_word) {
        ...
        if (identical(character(0),as.character(head(bg[bg$w1 == the_word[1], 2], 1)))) {
                # testing print(bg$w1)
                message<<-"If no word found using the highest occuring one gram word 'the'"
...
functionPentgram <- function(the_word) {
  # testing print(the_word)
  if (identical(character(0),as.character(head(pt[pt$w1 == the_word[1]
                                                  & pt$w2 == the_word[2]
                                                  & pt$w3 == the_word[3]
                                                  & pt$w4 == the_word[4], 5], 1)))) {
    # testing print of pt$w1, pt$w2, pt#w3, the_word[1], the_word[2], the_word3
    as.character(predictWord(paste(the_word[2],the_word[3],the_word[4],sep=" ")))
  }
  else {
    message <<- "Trying to Predict the Word using Quadgram Frequency Matrix"
    as.character(head(pt[pt$w1 == the_word[1] 
                         & pt$w2 == the_word[2]
                         & pt$w4 == the_word[4], 5], 1))
    # testing print of pt$w1, pt$w2, pt#w3, the_word[1], the_word[2], the_word3
  }       
